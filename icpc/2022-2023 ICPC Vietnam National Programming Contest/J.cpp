...
void solve(int n) {
	dp[1][0] = 1;
	for (int i = 2; i <= n; i++) {
		for (int h = 1; h < i; h++) {	///bat dau ghep i vao cac dinh j: 1 -> i - 1
			for (int k = 0; k < h; k++) {	// gia su dinh j dang ghep o do cao k -> i ghep vao se co do cao k + 1
				add(dp[i][k + 1], dp[i - 1][h - 1] * (1 << k), mod);			// h - 1 la do cao cua dinh j
			}
		}
	}
	ll ans = 0;
