// CONSIDER prefix sum p[0], p[1], .. p[n]
// A segment satisfied is some p[i], 2*p[i], 3*p[i]	so, count at p[i] / 2 if p[i] divisible by 2 (there is some p[j], j < i)	so that p[j] * 2 = p[i]
// remember to check for the last segment p[n] - p[i] = p[i] / 2
...
int32_t main() {
	...
	ll ans = 0;
	for (int  i = 0; i < n - 1; i++) {
		if (i == 0)	{dp[pref[i]]++;	continue;}
		if (pref[i] % 2 == 0 && pref[n - 1] - pref[i] == pref[i] / 2)	ans += dp[pref[i] / 2];
		dp[pref[i]] ++;
	}
	cout << ans;
	timer;
