...
void dfs(int u, int par = -1) {
	for (int v : a[u]) {
		if (v == par)	continue;
		dfs(v, u);
		if (color[u].size() < color[v].size())	swap(color[u], color[v]);
		for (auto &x : color[v])	color[u].insert(x);
	}
	ans[u] = color[u].size();
}
...
int main() {
	setIO("");
	cin >> n;
	ans.resize(n);	color.resize(n);	a.resize(n);
	for (int i = 0; i < n; i++) {
		int c;	cin >> c;
		color[i].insert(c);
	}
	...
	cout << '\n';
	for (int i = 0; i < n; i++)		debug(color[i]);
}